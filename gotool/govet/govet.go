...
func ExecCmd() {
	...
	cmd := exec.Command("/usr/local/go/bin/go", "tool", "vet", "./")
...
func GoVetMain() int {
	...
	if len(parameters) == 0 {
		parameters = append(parameters, gofmt.GetPath())
	}
	...
	if len(parameters) == 1 && strings.HasSuffix(parameters[0], "vet.cfg") {
		doPackageCfg(parameters[0])
		//os.Exit(exitCode)
		return exitCode
	}
	...
	if doPackage(parameters, nil) == nil {
